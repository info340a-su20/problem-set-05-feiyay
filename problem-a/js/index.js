...
let attalt = document.querySelector('img');
attalt.alt = 'A beautiful rainbow';

//Give the image the Bootstrap-provided `float-right` CSS class to make it float
//to the right of the screen
attalt.classList.add('float-right');
...
attributes.
*/
function createColorBox(color, number){
let div = document.createElement("div");
div.classList.add('d-inline-block');
div.style.backgroundColor = color;
div.style.width = `${number.toString()}px`;
div.style.height = `${number.toString()}px`;
...
function getElementWidth(dom){
  //var dom1 = document.querySelector(dom.toString());
  //var data = .getBdomoundingClientRect();
...
*/
function renderPaletteRow(string, parent){
let div = document.createElement("div");
for(var i=0;i<string.length;i++){
  div.appendChild(createColorBox(string[i],getElementWidth(parent)/string.length));
}
...
function renderPaletteTable(){
  ...
  let color = Object.keys(COLORS_9);
  for (var i=0;i<color.length;i++){
    renderPaletteRow(COLORS_9[color[i]],main);
  }
}
